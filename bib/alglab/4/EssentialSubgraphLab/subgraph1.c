...
/*  The initial version 1: memoization with a distance matrix  */  
...
typedef struct {            /* Maintain search status for nodes */ 
   int status;  
   double distance;     
 } searchtype; 
searchtype dnodes[MAXNODES];  /* For shortest-path search */ 
...
double curdist; 
...
#ifdef DEBUGSRC
 printf("start search unseen is %d \n", unseen); 
#endif 
 ...
 int i ; 
 for ( i = 1; i <= nodes; i++) {
   dnodes[i].status =unseen;
   dnodes[i].distance = INFINITY;  /* from math.h */ 
 } 

 dnodes[s].status  = inheap; 
 dnodes[s].distance = 0.0; 
...
#ifdef DEBUGSRC
   printf("\t Extract %d  %lf \n", x.hid, x.hval);
   if (x.hid == d) printf("\t found it %d  %lf \n", x.hid, x.hval); 
   fflush(stdout); 
#endif 

   if (x.hid == d) return x.hval;  /* found it */  

   dnodes[x.hid].status = seen;
   curdist = x.hval; 
...
#ifdef DEBUGSRC
     printf("\t %d has edge %d  %lf  status %d unseen %d \t", x.hid, ename, edist, dnodes[ename].status, unseen);
     fflush(stdout); 
#endif 

     if ( dnodes[ename].status == unseen) {

       heapinsert( ename, edist) ; 
       dnodes[ ename].status = inheap;
       
     }
     else if ( dnodes[ename].status == inheap ) {
...
int main(int argc, char** argv)
{
  ...
  for( tr= 0 ; tr< trials; tr++) {
    ...
    dminit(); /* V1: initialize the distance matrix*/ 
