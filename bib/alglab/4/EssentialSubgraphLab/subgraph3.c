...
/*  Version 3: memoization with a distance matrix, plus filter  */  
/* Cleaned up version2, but no time changes. */ 
/*  Version 3 also includes BFS with loop abort and filter */ 
...
#define MAXNODES 2001
#define MAXEDGES 2001000
...
int    queue[MAXNODES]; // bfs queue
double qdist[MAXNODES];  //bfs queue 
int    qstat[MAXNODES]; // status in queue or not 
double ubdiam; 
/*-----bfs-------------------------------------------------------*/
/*---------------------------------------------------------------*/ 

double bfs (int s, int d, double ecost, int rule) { 
  // search from s to d in the graph, using BFS. 
  // 1. up to limit ecost, or INFTY, filling in dmatrix for s. 
  // 2. up to finding d, filling in dmatrix for s. 
  // 3. up to component size, filling in dmatrix for row s.  Return distance to d if found.  if < nodes, diam ub is infty; or ub is twice max distance. 

  int inqueue = 1; 
  int outqueue = 0 ; 
  int i; 
  for (i = 1; i <= nodes; i++) qstat[i] = outqueue; 

  queue[0] = s;  
  qdist[0] = 0.0; 
  qstat[s] = inqueue; 
  int qfront = 0;  // inv: location of first item in q 
  int qback =  1;  // inv: next item goes here 

  int curnode;  
  double curdist; 
  double maxdist = 0.0; 

  double sddist = INFTY;  // default incase not found 
  //  ubdiam = INFTY;         // default incase not connected 
  int nodecount = 0; 

#ifdef DEBUGBFS
  printf("bfs %d %d %lf %lf \n" , s, d, ecost, ubdiam); 
#endif 

  while ( qfront < qback ) {   /* while not empty */ 
    // inv: heap is in q[qfront .. qback-1] 

    curnode = queue[qfront];    // dequeue 
    curdist = qdist[qfront]; 
    qfront++; 
    nodecount++; 

#ifdef DEBUGBFS
      printf("\t dm %d %d %lf   %lf \n", s, curnode, dmatrix[s][curnode], curdist); 
#endif 

    // update dmatrix 
    if ((dmatrix[s][curnode] == INFTY) ||  (dmatrix[s][curnode] > curdist)) { //every distance found is an ub on true distance-- make it better
      dmatrix[s][curnode] = curdist;
      dmatrix[curnode][s] = curdist;
    }

    if (curnode == d) {     // found it
      sddist = curdist; 
      if (rule == 2) return sddist;      
    }
    if (curdist > ecost) { 
      if (rule == 1) return curdist;    // dist will be bigger than ecost (non-essential).  Return curdist.  
    }
    edgetype *nptr; 
    int ename; 
    double edist; 

   /* for each neighbor e of curnode: add distance and put in queue*/ 

   nptr = nodelist[curnode].first;  
   while (nptr != NULL) {
     ename = nptr->dest;
     edist = curdist + nptr->cst;

     if (edist > maxdist) maxdist = edist;  // longest distance s to any x. 
     if ( qstat[ename] == outqueue ) {  // ename not in queu already 
  ...
  // queue is empty, and we haven't returned yet. Either not connected to d, or rule 3 applies. 
  // either way sdist is correct finite (if found) or infinite if not found

  if (nodecount >=  nodes-1) {
    if (2*maxdist < ubdiam) ubdiam = 2*maxdist; 
  } 
#ifdef DEBUGBFS
  printf("\t diam %lf \n", ubdiam); 
#endif 
...
int diamcheck[MAXNODES]; 
int bfsworks = 0; 
/*-----distance: search from s to d in E, return distance or infty      */ 
/*                                                                      */ 
double distance(int s,int d, double ecost,  int ssize )  {
  ...
  /*V1: memoization*/ 
  if ((dmatrix[s][d] != INFTY) && (dmatrix[s][d] < ecost)) 
     return dmatrix[s][d]; 

  double sddist = INFTY; 

  if (ssize >= nodes-1) { // if connected, bfs to find a ub on diameter, and a guess for sdist 
    if (diamcheck[s] == 0)  { // havent yet checked diameter for s 
      diamcheck[s] = 1; 
      sddist = bfs ( s, d, ecost, 3);  // return ub on d, also go to component and figure out ubdist 
      if ((sddist == INFTY) || (sddist <= ecost)) bfsworks ++; 
      if (sddist == INFTY ) return INFTY; // its essential 
      if (sddist <= ecost) return sddist; // its non-essential

    }//
  } // if 

  // if the sddist is INFINTY the edge is essential.  No Dijkstra search. 
  // else ssdist is finite. 
  // if the ub sddist is less than or equal to ecost, the edge is non-essential.  
  //           No dijkstra search.  
  //           (rule 3) continue on to check the diameter.  
  //           all distances found are ubs on final distance s to d:
  //           all distances found x via s to y are ubs on final distance: 
  //           If diameter ub is non-infinity, and less than ecost, the program is done. 
  // 
  // Dijkstra search, using ecost to filter. 
  ...
  return INFTY;  /* didn't find d reachible from s */  
...
void dminit() { 
  ...
  for (i = 1; i <= nodes; i++) diamcheck[i] = 0; 
  for (i = 1; i <= nodes; i++) 
    for (j= 1; j <= nodes; j++) dmatrix[i][j] = INFTY; 
} 
...
int main(int argc, char** argv)
{
  ...
  for( tr= 0 ; tr< trials; tr++) {
    ...
    while (!DONE) {        /* generate the subgraph */    
      ...
      nextedge(&v, &w, &ecost);    
      edgecount++; 
      mindist = distance(v,w, ecost, insertcount);

#ifdef DEBUG
      printf("edge %d : (%d  %d %lf) dist %lf ", edgecount, v,w,ecost, mindist);
      if (mindist == INFTY) printf("ins \n"); 
     ...
     if ( (mindist == INFTY)|| (mindist > ecost)) { 
            insert(v,w,ecost); 
            insertcount++; 
            lastcost = ecost;
	    lastrank = edgecount; 
